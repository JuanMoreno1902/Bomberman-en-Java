...
public class Floor {
	...
	// Función para guardar nombres e ID
	private String nombre;
	private int id;

	public void Player(String nombre, int id) {
		this.nombre = nombre;
		this.id = id;
	}
	...
	//
	// CONTADOR CUANDO SE DESTRUYE UN BLOQUE
	...
	public int getTotalScore() {
		...
		return getEnemyCount() + blockCounter;
	}
	...
	private void spawnPowerup(int rowIndex, int colIndex) {
		...
		if (r < CHANCE_FOR_RADIUS_POWERUP) {
			powerupList.add(new BombRadiusPU(squareToPixel(rowIndex) + BombermanComponent.getSquareMiddle(),
					squareToPixel(colIndex) + BombermanComponent.getSquareMiddle()));
		} else if (r > CHANCE_FOR_COUNTER_POWERUP) {
			powerupList.add(new BombCounterPU(squareToPixel(rowIndex) + BombermanComponent.getSquareMiddle(),
					squareToPixel(colIndex) + BombermanComponent.getSquareMiddle()));
		}
	}
	...
	private void spawnEnemies(int nrOfEnemies) {
		for (int e = 0; e < nrOfEnemies; e++) {
			while (true) {
				...
				if (randRowIndex == 1 && randColIndex == 1 || randRowIndex == 1 && randColIndex == 2
						|| randRowIndex == 2 && randColIndex == 1) {
					continue;
				}
				if ((randRowIndex % 2) == 0) {
					enemyList.add(new Enemy(squareToPixel(randColIndex) + BombermanComponent.getSquareMiddle(),
							squareToPixel(randRowIndex) + BombermanComponent.getSquareMiddle(), true));
				} else {
					enemyList.add(new Enemy(squareToPixel(randColIndex) + BombermanComponent.getSquareMiddle(),
							squareToPixel(randRowIndex) + BombermanComponent.getSquareMiddle(), false));
				}
				break;
			}
		}
	...
	public boolean collisionWithEnemies() {
		for (Enemy enemy : enemyList) {
			if (collidingCircles(player, enemy.getX() - BombermanComponent.getSquareMiddle(),
	...
	public boolean collisionWithBombs(AbstractCharacter abstractCharacter) {
		...
		for (Bomb bomb : bombList) {
			...
			if (playerLeftBomb && collidingCircles(abstractCharacter, squareToPixel(bomb.getColIndex()),
	...
	public void collisionWithPowerup() {
		for (AbstractPowerup powerup : powerupList) {
			if (collidingCircles(player, powerup.getX() - BombermanComponent.getSquareMiddle(),
